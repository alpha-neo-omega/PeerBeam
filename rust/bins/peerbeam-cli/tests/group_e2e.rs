...
//!
//! # It then caught a second one, and was withdrawn for it
//!
//! With the receiving half wired this test still failed, on Linux, Windows and
//! macOS, and it was taken out (`ac8874e`) on the assumption that a test failing
//! everywhere was a bad test. It was not. `groups.rs::with_device` ended with
//! the *non-draining* `session.handle.close()`, which only posts a close command
//! and returns, so the command's process could exit before the frame it had
//! just "sent" reached the wire. `send_on_channel` returning `Ok` means the
//! session actor accepted the payload — not that the peer holds it.
//!
//! It reproduced 3/3 locally once looked at properly, and passes 5/5 against the
//! draining `close().await`. The same defect was in four of the app's FFI paths
//! (ring, receipt, reaction, notes) and in group control, all fixed with it.
//!
//! So this test guards two things now: that the receiving half exists, and that
//! the sending half waits long enough for it to matter.
